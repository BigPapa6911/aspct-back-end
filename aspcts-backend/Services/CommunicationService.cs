...
public class CommunicationService : ICommunicationService
{
    ...
    {
        // Get sender user to determine role
        var senderUser = await _userRepository.GetByIdAsync(senderId);
        if (senderUser == null)
            throw new ArgumentException("Usuário remetente não encontrado");
            
        // Get receiver user to determine role
        var receiverUser = await _userRepository.GetByIdAsync(request.ReceiverId);
        ...
        var canSenderAccess = await CanAccessChild(request.ChildId, senderId, senderUser.Role);
        var canReceiverAccess = await CanAccessChild(request.ChildId, request.ReceiverId, receiverUser.Role);
    ...
    {
        if (!await CanAccessChild(childId, userId, userRole))
    ...
    public async Task<IEnumerable<MessageResponse>> GetConversationAsync(Guid otherUserId, Guid childId, Guid userId)
    {
        ...
        if (!await CanAccessChild(childId, userId, user.Role))
            return new List<MessageResponse>();
